    ...
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isPortfolio 
            ? (scrolled ? 'bg-black/90 backdrop-blur-md shadow-sm py-4' : 'bg-black py-6')
            : (scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6')
        } px-6 lg:px-12 flex items-center justify-between font-sans`}
      >
        <div className="flex-1 flex justify-start">
          <Link href="/" className="flex items-center gap-3 group">
             <img src={isPortfolio ? "/logo-white.svg" : "/logo-black.svg"} alt="Asterix Studio Logo" className="h-[38px] w-auto object-contain transform group-hover:scale-105 transition-transform" />
          </Link>
        </div>
        ...
        <div className="flex-1 flex justify-end items-center gap-4 md:gap-5">
           ...
           <Link
             ...
             target="_blank"
             className={`hidden md:flex ${isPortfolio ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'} px-6 py-2.5 rounded-full text-[13px] font-semibold transition-all items-center gap-2`}
           >
             Konsultasi Gratis
           </Link>
           ...
           <button
             ...
             className={`text-xl ${isPortfolio ? 'text-white hover:text-emerald-400' : 'text-gray-900 hover:text-emerald-600'} transition-colors p-2 lg:hidden`}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
             <div className="flex items-center justify-end px-6 py-6 md:px-12 relative z-50">
                <button
            ...
            <div className="flex-1 w-full h-full -mt-20">
                <FlowingMenu 
                    items={navItems}
                    bgColor="#000000"
                    textColor="#ffffff"
            ...
            <div className="p-8 pb-12 flex justify-center border-t border-white/10 z-40 bg-black">
               <Link
                 href="https://wa.me/6285129488941?text=Halo%20Asterix%20Studio,%20saya%20ingin%20konsultasikan%20website%20saya."
                 target="_blank"
                 onClick={() => setIsOpen(false)}
                 className="w-full text-center bg-white text-black px-8 py-4 rounded-full text-lg font-bold hover:bg-gray-200 transition-colors"
               >
                 Konsultasi Gratis
               </Link>
            </div>
